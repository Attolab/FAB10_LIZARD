...
class DAQ_1DViewer_LecroyWaverunner6Zi(DAQ_Viewer_base):
    ...
    def ini_detector(self, controller = None):
        ...
        try:
            ...
            self.controller.WriteString(
                ...
                1)
            self.numSegments = self.controller.ReadString(8)

            self.controller.WriteString(
                r"""vbs? 'return=app.acquisition.horizontal.samplemode' """, 1)
            self.sampleMode = self.controller.ReadString(8)
            ###########################################################
            ###########################################################
            return self.status
    ...
    def grab_data(self, Naverage=1, **kwargs):
        ...
        if self.debug:
            print('enter grab_data', time.perf_counter())
        ...
        if self.debug:
            print('after get waveform', time.perf_counter())

        if self.sampleMode == "Sequence":
            while True:
                ...
                if acquiredSegments == self.numSegments:
                    break
        ...
        if self.debug:
            self.controller.WriteString(
                ...
                1)
            acquiredSegments = self.controller.ReadString(8)
            print('acquired segments', acquiredSegments)
            print('out grab_data  ', time.perf_counter())
